<template>
  <!-- prettier-ignore -->
  <section id="faqs" class="w-full min-h-screen overflow-hidden px-5 lg:px-32 block flex items-center justify-between text-dark relative" style="background-color:#fbfcf4">
    ...
    <div class="hidden 2xl:block w-full" style="filter:invert(1)">
      <video autoplay muted loop class="w-2/3 xl:w-full max-w-2xl block m-auto relative xl:float-right" src="/an3.mp4" loading="lazy"></video>
    </div>
  </section>
</template>
<script>
    ...
    return {
      ...
      faqs: [
        ...
        },
        {
          ...
          answer:
            'PG operates on a referral basis where you can only join the group via an existing member.',
        },
        {
